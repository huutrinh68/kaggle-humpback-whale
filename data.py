...
class SiameseDataset(Dataset):
    @data_ingredient.capture
    def __init__(self, data_reader, mode, batch_size, same_ratio, path, image_size):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            b = self.data_reader.read_for_training(b).copy()
        else:
            a = self.data_reader.read_for_validation(a).copy()
            b = self.data_reader.read_for_validation(b).copy()

        return self.preprocess(a), self.preprocess(b), c
