...
def mul_data_train(epoch, model, criterion, optimizer, train_loaders, args):
    ...
    freq = min(args.print_freq, len(train_loaders[0]))

    # train 2 data-loaders
    for i, data_ in enumerate(zip(train_loaders[0], train_loaders[1]), 0):
        _0_data, _1_data = data_

        _0_inputs, _0_labels = _0_data
        _1_inputs, _1_labels = _1_data

        _0_labels = -_0_labels

        inputs = torch.cat([_0_inputs, _1_inputs], 0)
        labels = torch.cat([_0_labels, _1_labels], 0)
        ...
        if (i + 1) % freq == 0 or (i + 1) == len(train_loaders[0]):
            print('Epoch: [{0:03d}][{1}/{2}]\t'
                  'Time {batch_time.avg:.3f}\t'
                  'Loss {loss.avg:.4f} \t'
                  'Accuracy {accuracy.avg:.4f} \t'
                  'Pos {pos.avg:.4f}\t'
                  'Neg {neg.avg:.4f} \t'.format
                  (epoch + 1, i + 1, len(train_loaders[0]), batch_time=batch_time,
                   loss=losses, accuracy=accuracy, pos=pos_sims, neg=neg_sims))
